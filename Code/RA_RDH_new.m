...
function res = RA_RDH1( origin )
    res = origin;
    [M,N,C] = size(origin);
    for channel = 1 : 1 : C
        bits = [];
        [bits,~] = Processing(bits,0,origin(:,:,channel),M,N);
        [~,l] = size(bits);
        capacity = M*N*8
        length = l
...
function [bits,I] = Processing( bits, k, I, M, N )
    if k < 6
        [I_Ad, Lval] = Adoptation_MED(k,I,M,N);
%         for i = 1 : 1 : M*N
%             if Lloc(i) ~=0
%                 i,Lloc(i)
%             end
%         end
        ...
        if l1 >= floor((M*N-25)/(8-k))
            for i = k : 7
                bit_I = bitand(I,2^(7-i));
                bit_I = bit_I';
                bit_I = bit_I(:);
                bits = [bits, bit_I', 0];
            end
            bits = [bits, dec2bin(I(1,1),8)-'0'];
            return;
        else
%             I = Adoptation(I,k,Lloc,Lval);
            ...
            Lval = Lval(:);
            [length,~] = size(Lval);
            bits = [bits, Lval', Dec2bin(length,24)-'0', 1];
            [bits,I] = Processing(bits,k+1,I_Ad,M,N);
        end
    else
        bit_I = bitand(I,2);
        bit_I = bit_I';
        ...
        bit_I = bit_I';
        bit_I = bit_I(:);
        bits = [bits, bit_I', 0, dec2bin(16,8)-'0'];
    end 
end
...
%I: the original image
function [I_Ad, Lval] = Adoptation_MED(k,I,M,N)
    I_Ad = I;
    Lloc = zeros(1,M*N);
    Lval = [];
...
%             end
            ...
            if i == 1
                x1 = I(i,j-1);
                pred = Prediction(0, x1, 0, 1, k);
            elseif j == 1
                x0 = I(i-1,j);
                pred = Prediction(x0, 0, 0, 0, k);
            else
                x0 = I(i-1,j); x1 = I(i,j-1); x2 = I(i-1,j-1);
                pred = Prediction(x0, x1, x2, 2, k);
            end
            inv = bitxor(p,2^(7-k));
            delta = abs(pred - p);
            deltai = abs(pred - inv);
...
%                 Lloc(nol) = 1;
            ...
            if Lloc((j-1)*M+i) ~= 0
                if abs(Lval(index)) ~= 2^(6-k)+1
                    I_Ad(i,j) = mod(int32(Ii) + int32(Lval(index)),256);
                end
                index = index + 1;
            end  
        end
...
function pred = Prediction(x0, x1, x2, type, k)
%     x0 = origin(x-1,y);
%     x1 = origin(x,y-1);
%     x2 = origin(x-1,y-1);
    if type == 0 % x-1 -> x
